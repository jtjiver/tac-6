...
5. **If a slash command completes** (e.g., /review, /patch, /implement, /commit), immediately continue with the next step
6. **Display final summary only** when Step 9 is marked "completed" in your TodoWrite list

**Why this matters:** The automated system (`adws/adw_review.py`) runs all steps sequentially without pausing. This interactive guide must match that behavior to provide the same experience.
